...
def register():
    ...
    if form.validate_on_submit():
        ...
        send_email(user.email, 'email_confirm', 'auth/email/confirm', user=user,
                   token=token)
...
def resend_confirmation():
    ...
    send_email(current_user.email, 'email_confirm', 'auth/email/confirm',
...
def change_password():
    ...
    if form.validate_on_submit():
        if current_user.verify_password(form.oldpassword.data):
            ...
            flash('Your password have been change now!')
            return redirect(url_for('main.index'))
        else:
...
def change_email_request():
    ...
    if form.validate_on_submit():
        if current_user.verify_password(form.password.data):
            ...
            send_email(new_email, 'email_confirm',
...
def password_reset_request():
    if not current_user.is_anonymous:
        return redirect(url_for('main.index'))  # 保护路由，只有没有登录的才能访问
